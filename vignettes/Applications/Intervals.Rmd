...
ov_bt_filtered %>% 
  dplyr::select(rx, log_scale) %>% 
  tidyr::gather(statistic, coefficients) %>% 
...
The departures from normality may account for the difference in the intervals (and would suggest that the nonparametric estimates are advisable). 
...
The _t_-intervals require the names of the variance columns (assumed to be in the same order as the columns with the individual statistics). These names can be wrapped inside of a `dplyr::vars()` function: 
