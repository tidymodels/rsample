...
For bias-corrected and accelerated (BCa) intervals, a different type of argument is required. The `fn` argument is a function that computes the statistic of interest. The first argument should be for an `rsplit` object and other arguments can be passed in using the ellipses. For example, to get the `rx` coefficient for the model:
