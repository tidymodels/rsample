...
Calibration is a transformation of the predictions from our primary model, based on a separate calibration model. That calibration model is used to model the relationship between the predictions based on the primary prediction model and the true outcomes.

Crucial: To avoid overfitting, we cannot use the same data to fit our calibration model as we use to assess the combination of prediction and calibration model. Using the same data to fit the prediction model and the calibration model means the predictions used to fit the calibration model are re-predictions of the same observations used to fit the prediction model. Hence they are closer to the true values than predictions on new data would be and the calibration model doesn't have accurate information to estimate the right trends (so that they then can be removed).
...
If we include calibration into our workflow (bundeling preprocessing, (prediction) model, and post-processing), we want an analysis set, a calibration set, and an assessment set.
...
"Taking data from the analysis set" means splitting up the analysis set to end up with ... an analysis set and a calibration set. Now we have two sets call analysis set, that's confusing. If we need to distinguish them, we'll refer to them as "outer" and "inner" analysis set for "before" and "after" the split for a calibration set.

![](images/analysis-calibration-assessment.jpg)

It is important to note that including calibration requires more data than fitting a workflow without calibration. For small datasets, you might run into situations where fitting either model (prediction model or calibration model) is not particularly stable - or you might not even be able to construct a calibration set properly. In this case, our policy is to avoid erroring. We would treat this situation as if no calibration model was used.

But how is the (outer) analysis set split if we need a calibration set?

If you want control over how exactly your calibration set is constructed, specify it in your tailor (or the calibration adjustment?). Here, we are going to lay out what happens as the default.

<!-- TODO: talk about how we have `inner_split()` methods that are being used by tune? --- Probably not here. -->
...
Note: the `inner_split()` methods are exported to be used by tune but not intended to be user-facing. 
...
i_split <- inner_split(r_split, .get_split_args(r_set))
i_split
# similar proportion of observations allocated for fitting the model in the inner split
nrow(analysis(i_split)) / nrow(analysis(r_split))
```
...
To mimick the split into (outer) analysis and assessment set, we'll calculate the proportion of the analysis set and apply it the observations available for fitting both the main prediction model and the calibration model, creating our (inner) analysis set. Since the remaining observations could be for the calibration model or a gap, we also calculate the proportion of the assessment set and use it to construct the calibration set. 
...
Side note: we'll introduce a `calibration()` accessor function for clarity. Here, we are using the existing `assessment()` acessor function but the data set returned by `assessment()` is the calibration set.

```{r}
#| label: sliding_window_inner_split
...
When creating the calibration split, we apply the `complete` option to the (now inner) analysis set and ensure that the calibration set is complete. In this example, it means that we can't do a calibration split until we have at least 4 rows, i.e., a calibration split on the second resample fails.
...
A calibration split on the third resample, which contains four rows, succeeds with the required three rows in the calibration set and the remaining one in the incomplete analysis set.
...
This is the index value relative to which we can define an inner analysis set of 3 and a calibration set of 2. Thus we cannot actually make that split.
...
inner_split(r_split, .get_split_args(r_set))
```
